...
const VERCEL_BLOB_VIDEO_BASE = 'https://plaqx6laeb8fwb3j.public.blob.vercel-storage.com'

const useVercelBlobVideos =
  process.env.NODE_ENV === 'production' &&
  (process.env.NEXT_PUBLIC_VERCEL_ENV === 'production' || process.env.VERCEL_ENV === 'production')

function getScenarioVideoSrc(fileName: string): string {
  if (useVercelBlobVideos) {
    return `${VERCEL_BLOB_VIDEO_BASE}/${fileName}`
  }

  return `${LOCAL_VIDEO_BASE}/${fileName}`
}
...
export const scenarios: Scenario[] = [
  {
    ...
    hint: 'My neighbor works at a hospital in Munich and said they are preparing extra beds',
    hintTiming: 5,
    videoSrc: getScenarioVideoSrc('Hantavirus.mp4'),
  ...
  },
  {
    ...
    hint: 'My cousin joined last week and already made back his deposit 🔥',
    hintTiming: 5,
    videoSrc: getScenarioVideoSrc('trading.mp4'),
  ...
  },
  {
    ...
    hint: 'They posted an update yesterday, the first families already received help ❤️',
    hintTiming: 5,
    videoSrc: getScenarioVideoSrc('charity.mp4'),
  ...
  },
  {
    ...
    hint: 'ALDI near me already pulled these from the shelves',
    hintTiming: 5,
    videoSrc: getScenarioVideoSrc('Cola2.mp4'),
...
'Product safety warnings often feel urgent and trustworthy because they use official-looking language, alarming visuals, and familiar brands. Keep in mind that deepfakes are not always created for direct financial gain, they may also be used to spread misinformation or damage a brand’s reputation.',
  ...
  },
  {
    ...
    hint: 'My uncle works at VW and says everyone there has been worried for weeks',
    hintTiming: 5,
    videoSrc: getScenarioVideoSrc('real.mp4'),
